...
function filter_articles($query) {
...
function get_manual_anchors_from_content($content) {
    // Use regex to find all <a> tags with href attributes that contain #
    preg_match_all('/<a[^>]+href=["\']#([^"\']+)["\'].*?>(.*?)<\/a>/', $content, $matches);

    // Check if any matches are found
    if (!empty($matches[1])) {
        echo '<ul class="manual-anchor-links">';
        foreach ($matches[1] as $key => $anchor) {
            echo '<li><a href="#' . esc_attr($anchor) . '">' . esc_html($matches[2][$key]) . '</a></li>';
        }
        echo '</ul>';
    }
}
